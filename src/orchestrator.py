...
class HorizonOrchestrator:
    ...
    async def run(self, force_hours: int = None) -> None:
        ...
        try:
            if self._placeholder_summary_enabled():
                self._generate_placeholder_outputs()
            ...
            for lang in self.config.ai.languages:
                ...
                self._save_summary_outputs(today, lang, summary)
    ...
    @staticmethod
    def _placeholder_summary_enabled() -> bool:
        return os.getenv("HORIZON_GENERATE_PLACEHOLDER_SUMMARY", "").strip() == "1"

    def _generate_placeholder_outputs(self) -> None:
        today = datetime.utcnow().strftime("%Y-%m-%d")
        self.console.print("[yellow]Placeholder summary mode enabled. Skipping fetch and AI steps.[/yellow]\n")
        for lang in self.config.ai.languages:
            summary = self._placeholder_summary(today, lang)
            self._save_summary_outputs(today, lang, summary)
        self._update_mobile_manifest()

    @staticmethod
    def _placeholder_summary(date: str, language: str) -> str:
        title = "Horizon Placeholder Summary" if language == "en" else "Horizon 占位摘要"
        body = (
            "This file was generated by GitHub Actions for deployment testing."
            if language == "en"
            else "这个文件由 GitHub Actions 生成，用于验证部署与 manifest 流程。"
        )
        return f"# {title}: {date}\n\n{body}\n"

    def _save_summary_outputs(self, date: str, lang: str, summary: str) -> None:
        summary_path = self.storage.save_daily_summary(date, summary, language=lang)
        ...
        with open(dest_path, "w", encoding="utf-8") as f:
            f.write(front_matter + self._strip_summary_h1(summary))

        self.console.print(f"📄 Copied {lang.upper()} summary to GitHub Pages: {dest_path}\n")

    @staticmethod
    def _strip_summary_h1(summary: str) -> str:
